...
function showMask(opera){
    $('#mask').remove();
    $('body').append('<div id="mask"'+(opera==1?"onclick=\'dlctipbox.clear()\'":"")+'style="position: fixed;cursor:pointer;height: 100%;width: 100%;top:0;background:rgba(0,0,0,0.6);z-index: 100;"></div>');
}
function hideMask(){
    $('#mask').remove();
}
...
function upLoadImg(file,picbox,size){
    ...
    for(var i = 0; i < file.files.length; i++) {
        ...
        if(ext != ".BMP" && ext != ".PNG" && ext != ".GIF" && ext != ".JPG" && ext != ".JPEG") {
            ...
            img_html = "<div class='isImg' style='height: 100%'><img src='" + objUrl + "' onclick='javascript:lookBigImg(this)' style='height: 100%; width: 100%;' /><button class='removeBtn' onclick='javascript:removeImg(this)' style='position:absolute;right: 0;top: 0;background: rgba(0,0,0,0.2);color: #fff;border-radius: 50%;width: 0.3rem;height: 0.3rem;font-size: 0.3rem;display: flex;align-items: center;justify-content: center;'>x</button></div>";
...
function dlcUrl(){
    return 'http://www.xiguagongxiang.cn';
}
...
function isLogin(cb){
    if(!token){//获取不到token
        // if(getUrlParam('code')==''){
        //     if(window.navigator.userAgent.toLowerCase().match(/MicroMessenger/i)){//如果是微信浏览器
        //         location.replace("https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxa965f1b3dca9b4ae&redirect_uri="+window.location.href+"&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect");
        //     }else{
        //         dlctipbox.show('请在微信环境下使用');
        //     }
        // }else{
        //     dlc_request('/yzshg/index/weixin_login?code='+getUrlParam('code'),{},function(res){
        //         console.log(res);
        //         if(res.code == 1){
        //             user_save(res.data.token);
        //             if(cb)cb({flag:1});//flag==1 登录成功
        //         }else{
        //             console.log(res.msg);
        //             if(cb)cb({flag:0});//flag==0 登录失败
        //         }
        //     })
        ...
        }else if(isWX_Allipay()=='Allipay'){//如果是支付宝客户端
            if(getUrlParam('auth_code')==''){
               location.href='https://openauth.alipay.com/oauth2/publicAppAuthorize.htm?app_id=2018022302257885&scope=auth_userinfo&redirect_uri='+encodeURIComponent(window.location.href);
            }else{
                dlc_request('/yzshg/index/ali_login?code='+getUrlParam('auth_code'),{},function(res){
                    // alert(getUrlParam('auth_code'));
